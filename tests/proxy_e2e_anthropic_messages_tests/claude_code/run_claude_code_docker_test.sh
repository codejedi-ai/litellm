...
for _ in $(seq 1 "${STARTUP_TIMEOUT_S}"); do
  if curl -fsS -H "Authorization: Bearer ${LITELLM_MASTER_KEY}" "${PROXY_URL}/v1/models" >/dev/null 2>&1; then
    READY=1
    break
  fi
  sleep 1
...
curl -fsS -H "Authorization: Bearer ${LITELLM_MASTER_KEY}" "${PROXY_URL}/v1/models" | grep -q "\"${MODEL_NAME}\""

echo "[4/5] Running Claude Code in an isolated non-root container..."
docker compose -p "${COMPOSE_PROJECT_NAME}" -f "${COMPOSE_FILE}" run --rm claude-code
